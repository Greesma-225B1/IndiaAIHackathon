# IndiaAI Hackathon – NLP Text Classification

An **NLP-based text classification project** developed as part of the **IndiaAI Hackathon**, focusing on analyzing and classifying textual data using Machine Learning and Deep Learning techniques.

---

## Problem Statement
The objective of this project is to build an efficient **text classification system** that can process raw text data and classify it into meaningful categories using NLP techniques.

---

## Features
- Text preprocessing using NLP techniques  
- Feature extraction from textual data  
- Machine Learning & Deep Learning models for classification  
- Performance comparison across different models  

---

## Tech Stack
- **Language:** Python  
- **Libraries:**  
  - NumPy  
  - Pandas  
  - Scikit-learn  
  - NLTK  
  - TensorFlow / Keras  
- **Environment:** Jupyter Notebook  

---

## Project Files
- `Text_Classification.ipynb` – Basic text classification  
- `Text_Classification_LSTM.ipynb` – LSTM-based model  
- `Text_Classification_BiLSTM.ipynb` – BiLSTM model  
- `Text_Classification_NLTK_PP.ipynb` – NLP preprocessing using NLTK  

---

## Dataset
The dataset used for this project includes:
- `train.gz` – Training data  [train.gz](https://github.com/user-attachments/files/18574056/train.gz)
- `test.csv` – Test data  [test.csv](https://github.com/user-attachments/files/18574001/test.csv)

> Dataset files are included in the repository for reproducibility.

---

## Methodology
1. Data loading and cleaning  
2. Text preprocessing (tokenization, stopword removal)  
3. Feature extraction  
4. Model training and evaluation  
5. Comparison of results  

---

## Future Enhancements
- Implement transformer-based models (BERT)  
- Improve accuracy using hyperparameter tuning  
- Deploy as a web application  

---

## Author
**Greesma**  
Final-year CSE Student | NLP & Machine Learning Enthusiast  

🔗 GitHub: https://github.com/Greesma-225B1



